...
2. **迁移到 ROS 2 (rclpy)**，并**订阅**与 `all_topics_scripts.py` 同名的 5 个话题：
...
# ------------------ 保存目录 ------------------
ROOT_SAVE_DIR = os.path.expanduser('~/data/ros2_record')
...
class RecordRobotData(Node):
    ...
    def __init__(self):
        ...
        self.cache = {
            'timestamp': [],          # str
            'wireless': [],          # unitree_go/msg/WirelessController | None
        ...
        # ---- 定时器：50 Hz —— 每次调用 record_tick ----
        self.create_timer(0.02, self.record_tick)

        self._stopped = False  # 一旦保存后置 True，忽略后续定时器

    # ---------- 各话题回调 ----------
    def cb_wireless(self, msg: WirelessController):
        self.latest['wireless'] = msg
