...
def load_vision_data(df):
    """Load ArUco vision data from CSV: keep missing as NaN so the plot breaks."""
    aruco_col = "aruco_x"
    if aruco_col not in df.columns:
        # 若无此列，用 NaN 占位（这样曲线整段不画）
        aruco_x = np.full(len(df), np.nan, dtype=float)
    else:
        # 非法/空字符串 -> NaN；不要填 0
        aruco_x = pd.to_numeric(df[aruco_col], errors="coerce").to_numpy()
...
def plot_figure2_metrics(t_yaw, yaw_vel, t_all, y_shifted_signed, t_aruco, aruco_x, 
    ...
    # Center ArUco X around first finite (non-NaN) value
    if len(aruco_c) > 0:
        finite_idx = np.where(np.isfinite(aruco_c))[0]
        if len(finite_idx) > 0:
            aruco_center = aruco_c[finite_idx[0]]
            aruco_c = aruco_c - aruco_center
        # 若全是 NaN，就不做居中，保持 NaN（曲线整段不画）
...
def main():
    ...
    out2 = args.out2 or base + '_vision_for_thesis2.png'
