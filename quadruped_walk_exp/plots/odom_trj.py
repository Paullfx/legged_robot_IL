...
def load_positions(csv_path, prefix='sport'):
    ...
    if col_x not in df.columns or col_y not in df.columns:
        for p in ['sport', 'lf_sport']:
            if f'{p}_pos1' in df.columns and f'{p}_pos0' in df.columns:
                col_x, col_y = f'{p}_pos1', f'{p}_pos0'
                break
        else:
            raise ValueError(f'Could not find position columns like \"{prefix}_pos1\" and \"{prefix}_pos0\" in {csv_path}')
...
def plot_traj(x, y, t, csv_path, out_path=None, title=None, timestamp_str=None):
    ...
    # Create a custom layout: left 1/3 for trajectory (full height), right 2/3 for 3 stacked plots
    ...
    # Plot y_shifted on x-axis and x_shifted on y-axis (no inversion)
    ax_traj.plot(y_shifted, x_shifted, '-o', markersize=2, linewidth=1)
    if len(x_shifted):
        # Always plot the start marker at (0, 0)
        ax_traj.scatter([0], [0], c='green', s=40, label='start')
        ax_traj.scatter([y_shifted[-1]], [x_shifted[-1]], c='red', s=40, label='end')
    # Set axis limits to use the full figure width/height
    if len(x_shifted):
        ax_traj.set_xlim(np.min(y_shifted), np.max(y_shifted))
        ax_traj.set_ylim(np.min(x_shifted), np.max(x_shifted))
    ax_traj.set_aspect('equal', adjustable='box')
    ax_traj.set_xlabel('y (pos0, m)')
    ax_traj.set_ylabel('x (pos1, m)')
    ax_traj.set_title('Trajectory (axes swapped)')
    ax_traj.legend()
    ax_traj.grid(True)
    ...
    ax_y.plot(t, y_shifted_time, label='y (pos0) - y[0]')
    if len(y):
...
def main():
    ...
    # allow user to pass an experiment folder name like exp_50hz_... and resolve all_topics.csv
    ...
    plot_traj(x, y, t, csv_path, out_path=args.out, title=None, timestamp_str=first_timestamp_str)
