...
def plot_figure1_traj(x, y, t, out_path, a4=False, dpi=300):
    ...
    ax.plot(y_abs_shifted, x_shifted, '-o', markersize=2, linewidth=2.5)
    if len(x_shifted):
        ax.scatter([0], [0], c='green', s=100, label='start', zorder=3)
        ax.scatter([y_abs_shifted[-1]], [x_shifted[-1]], c='red', s=100, label='end', zorder=3)

        # x=0.25 m 的水平参考线（纵轴是 x）
        ax.axhline(0.25, color='gray', linestyle='--', linewidth=1)

        # 自适应轴限，留一点边距
        pad_x = max(0.05, 0.05*np.nanmax(y_abs_shifted) if len(y_abs_shifted) else 0.05)
        pad_y = max(0.1, 0.05*np.nanmax(np.abs(x_shifted)) if len(x_shifted) else 0.1)
        ax.set_xlim(0.0, (np.nanmax(y_abs_shifted) if len(y_abs_shifted) else 0.75) + pad_x)
...
def plot_figure2_metrics(t_yaw, yaw_vel, t_all, y_shifted_signed, t_aruco, aruco_x, 
    ...
    cmd_vel_lin_y_c = cmd_vel_lin_y[mask_cmd_vel]
    ...
    ax_aruco.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=1, frameon=False)

    # 2) Yaw velocity (middle left)
    ...
    # Right column
    # 4) cmd_vel_lin_x (top right)
    ax_cmd_x = fig.add_subplot(gs[0, 1])
    ...
    ax_cmd_x.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=1, frameon=False)

    # 5) cmd_vel_ang_z (middle right)
    ax_cmd_ang = fig.add_subplot(gs[1, 1])
    ...
    ax_cmd_ang.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=1, frameon=False)

    # 6) cmd_vel_lin_y (bottom right)
    ax_cmd_y = fig.add_subplot(gs[2, 1])
    ax_cmd_y.plot(t_cmd_vel_c, cmd_vel_lin_y_c, linewidth=2.5, label='cmd_vel_lin_y')
    ax_cmd_y.set_xlim(0, clip_time)
    ax_cmd_y.set_xlabel('Time (s)')
    ax_cmd_y.set_ylabel('lin_y (m/s)')
    ax_cmd_y.set_title('Command Linear Velocity Y')
    ax_cmd_y.grid(True, alpha=0.4)
    ax_cmd_y.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=1, frameon=False)

    fig.savefig(out_path, dpi=dpi, bbox_inches='tight')
    print('Saved Figure 2 (six subplots with vision and cmd_vel) to', out_path)
    try:
        plt.show()
    except Exception:
        pass
...
def main():
    p = argparse.ArgumentParser(description='Make two figures: (1) trajectory-only, (2) six subplots including vision and cmd_vel data.')
    ...
    out2 = args.out2 or base + '_vision_metrics.png'
